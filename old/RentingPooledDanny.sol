...
uint256 deployDate;
    ...
    function rent(){
        uint256 maxRentalPerDay = (staked.length / 30) * rentalMultiplier;
        uint256 daysSinceDeploy = (block.timestamp - deployDate) / 1000 / 60 / 60 / 24;
        uint256 rentalMaxLimit = daysSinceDeploy * maxRentalPerDay;
    ...
    function getAmountAvailableForRental() public returns (uint256){
        uint256 daysSinceDeploy = (block.timestamp - deployDate) / 1000 / 60 / 60 / 24;
        uint256 rentalMaxLimit = daysSinceDeploy * maxRentalPerDay;
